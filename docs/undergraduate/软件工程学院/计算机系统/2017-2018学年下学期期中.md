...
假设一种整数为 6 位字长，根据第一列的描述，填写其对应的值。
...
考虑基于 IEEE 浮点格式标准的 5 位浮点表示，这种格式没有符号位，只表示非负数（阶码 3 位，尾数 2 位）。
...
在 X86-64 机型上，产生整型变量 `x`、`y`、`z` 的随机数，试判断下列 C 表达式是否恒为真。
...
考虑如下 C 语言代码，其中 `M` 和 `N` 是由 `#define` 声明的常量。
